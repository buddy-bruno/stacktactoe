...
export function GameHelpSidebar({ open, onClose }: GameHelpSidebarProps) {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-40 bg-black/40 backdrop-blur-sm md:bg-black/20 transition-opacity duration-300 ease-out ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        aria-hidden
        onClick={onClose}
      />
      <aside
